...
export default () => {
  ...
  const setDays = ((): (() => void) => {
    ...
    return (): void => {
      ...
      const currentMonthLastDay: Date = new Date(
        state.year,
        state.month + 1,
        0
      );
      ...
      const getPrevMonthDays = (day: Date): number[] =>
        Array.from(
          { length: day.getDay() },
          (_, i) => prevMonthLastDay.getDate() - prevMonthLastDay.getDay() + i
        );

      // eslint-disable-next-line max-len
      const getNextMonthDays = (day: Date): number[] =>
        Array.from({ length: 6 - day.getDay() }, (_, i) => i + 1);

      // eslint-disable-next-line max-len
      const getCurrentMonthDays = (day: Date): number[] =>
        Array.from({ length: day.getDate() }, (_, i) => i + 1);
  ...
  const render = (): void => {
    ...
    $calenderContainer.innerHTML =
      state.prevMonthDays
        .map(
          (day) =>
            `<button id="${day}" style="color: #aaa; opacity: 0; position: relative; z-index: -10;">${day}</button>`
        )
        .join('') +
      state.currentMonthDays
        .map(
          (day) =>
            `<button id="${day}" style="color: ${
      ...
      state.nextMonthDays
        .map(
          (day) =>
            `<button id="${day}" class="next-month" style="opacity: 0; position: relative; z-index: -10;">${day}</button>`
        )
        .join('');
  };
  ...
  $radioSection.addEventListener('change', (e) => {
    const eventTarget = <HTMLElement>e.target;
    ($radioSection.querySelector('.active') as HTMLElement).classList.remove(
      'active'
    );
    (eventTarget.closest('.radio-container') as HTMLElement).classList.add(
      'active'
    );
    state.time = (($radioSection.querySelector('.active') as HTMLElement)
      .textContent as string).trim();
  });
